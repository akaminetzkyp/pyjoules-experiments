...
def cifar10(root='./data'):
    try: 
        ...
        return {k: {'data': np.stack((np.pad(v.data, ((0, 0), (2, 2),  (2, 2)), mode='constant'),)*3, axis=-1), 'targets': v.targets} for k,v in [('train', download(train=True)), ('valid', download(train=False))]}
